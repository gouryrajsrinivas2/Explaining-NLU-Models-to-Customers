...
data_dict["twitter_3000"]["label"] = [i % 2 for i in range(len(data_dict["twitter_3000"]))]
...
def demo_callbacks(app):
    ...
    def generate_figure_word_vec(
        ...
        try:
            ...
            embedding_df["label"] = list(data_dict["twitter_3000"]['label'])
            ...
            for idx, embedding_df in groups:
                scatter = go.Scatter3d(
                    ...
                    # text=[idx for _ in range(embedding_df["x"].shape[0])],
    ...
    def display_click_word_neighbors(clickData, dataset):
        if dataset in WORD_EMBEDDINGS and clickData:
            ...
            try:
                # Get the nearest neighbors indices using Euclidean distance
                vector = data_dict[dataset].set_index("0")
                if 'label' in vector.columns:
                    labels = vector['label']
                    vector.drop('label', axis=1, inplace=True)
                selected_vec = vector.loc[selected_word]
                ...
                c = Counter([labels[labels.index == idx][0] for idx in nearest_neighbors.index])
                layout = go.Layout(
                    title="Intents in nearest neighbours",
                    margin=go.layout.Margin(l=60, r=60, t=100, b=0),
                    paper_bgcolor='rgba(0,0,0,0)'
                )
                pie_fig = go.Figure(data=[go.Pie(labels=list(c.keys()), values=list(c.values()),
                ...
                trace = go.Bar(
                    ...
                    y=[str(idx) + " --> class:" + str(labels[labels.index == idx][0]) for idx in
